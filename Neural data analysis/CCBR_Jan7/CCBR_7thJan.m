...
%figure;plot(lags*Dt,correlation)
...
%%  I would expect 1000 spikes in this spike train as there is one spike every 20ms
    %and hence in 20s there would be 1000 spikes. 
...
%expect to see?
[correlation2,lags2]=xcorr(Spikes2,'coeff');%xcorr function with a single vector as 
...
for i = 1:100
    ...
    FR=i; 
    Lambda=FR*Dt;
    Rand=rand([Timepoints,1]);
    Spikes=zeros([Timepoints,1]); 
    Spikes(Rand<Lambda)=1;
    SpikeIndices=find(Spikes==1); 
    SpikeTimes=SpikeIndices*Dt; 
    ISI=diff(SpikeTimes);
    meanISI(i) = mean(ISI);
    stdISI(i) = std(ISI);
end   
figure;scatter(meanISI, stdISI)
hold on
plot(stdISI, stdISI)
hold off
%% HW problems
...
Q5. (40 points) Create 2 spike trains, at a resolution of 1ms for 
